    ...
    var body: some Scene {
        MenuBarExtra {
            ...
            Button(delegate.panelVisible ? "Hide Buddy" : "Show Buddy") { delegate.toggle() }
                .keyboardShortcut("b")
            Toggle("Hot corner (top left)", isOn: Binding(get: { delegate.hotCorner }, set: { delegate.hotCorner = $0 }))
...
final class AppDelegate: NSObject, NSApplicationDelegate, ObservableObject {
    ...
    /// Hot-corner mode: the buddy stays faded out until the mouse hits the top-left corner of its screen,
    ...
    private func pollHotCorner() {
        ...
        let corner = CGPoint(x: screen.frame.minX, y: screen.frame.maxY)
